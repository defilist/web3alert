...
from hack.model import Rule, Receiver
from hack.postgres_receiver import PostgresReceiver
...
class AlertExporter:
    def __init__(
        ...
        self._chain = chain
        self._db: Session = sessionmaker(autocommit=False, autoflush=False, bind=engine)
        ...
        # self._price_service = SimplePriceService()
        self._price_service = None
        self._label_service = label_service
        if enable_print:
            self.printer = PrintReceiver()
        else:
            self.printer = None
        self.postgres_receiver = PostgresReceiver(engine)
    ...
    def get_ruleset(self) -> Tuple[Dict[str, Receiver], RuleSet]:
        receivers = {}
        rules = []
        with self._db() as conn:
            for r in conn.query(Receiver).all():
                typo = r.receiver
                init_args = r.init_args
                m = module(f"blockchainetl.alert.receivers.{typo}_receiver")
                c = getattr(m, f"{typo.title()}Receiver")
                receivers[r.name] = c(**init_args)
            for r in conn.query(Rule).filter_by(chain=self._chain).all():
                rules.append(
                    {
                        "id": r.name,
                        "scope": r.scope,
        ...
        ruleset = RuleSet.load(self._chain, None, rules)
        return receivers, ruleset

    def export_items(self, items: List[Dict]):
        if len(items) == 0:
            return

        receivers, ruleset = self.get_ruleset()
        full_items = self.restruct(items)

        rule_results = list(
            ruleset.execute(self._executor, full_items, self._max_workers)
        )
        ...
        for rule_id, result in rule_results:
            ...
            if self.printer is not None:
                self.printer.post(rule, result)

            self.postgres_receiver.post(rule, result)

            for rec, receiver in receivers.items():
